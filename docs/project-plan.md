...
| Q7 | **Preferred skills (8%) is the weakest component** — affects 1 of 3 profiles, replaces no top-5 job on average. Location (8%) is low but not inert (2 of 3 profiles) | Nothing shipped; reporting only | Only 7.0% of postings list preferred skills and AC-9.4 renormalises the weight away on the other 93%, so the component rarely fires. Measured across all three profiles, not one — an earlier reading called location inert too, which was an artifact of testing a single profile whose top results are mostly remote. Recorded as a measured limitation rather than re-tuned without time to re-validate every AC; the weights also trace directly to the Stage 1 and Stage 2 documents. Report §12 future work |
